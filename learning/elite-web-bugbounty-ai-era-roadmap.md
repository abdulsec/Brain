...
## Logic Bug Mastery Resources And Mindset

Logic bugs are not mainly about payloads. They come from understanding what the application promises to enforce, then finding where the product lets you violate that promise.
...
- What is the expected business rule?
- Who is allowed to perform this action?
- What object is being acted on?
- What state should the object be in?
- What previous step must have happened?
- What next step depends on this one?
- What happens if I skip, repeat, reverse, race, or replay this step?
- What happens if I use a different role, tenant, API version, mobile API, or old token?
...
PortSwigger's race condition research is mandatory reading for advanced business logic:

- Smashing the state machine: https://portswigger.net/research/smashing-the-state-machine

Race bugs are business logic bugs where timing lets you enter an invalid state.

### Blogs And Research To Read
...
- SonarSource Research: https://www.sonarsource.com/blog/
- HackerOne Hacktivity: https://hackerone.com/hacktivity
- BugBoard: https://bugboard.rsecloud.com/
- Bugcrowd Blog: https://www.bugcrowd.com/blog/
- Intigriti Researcher Blog: https://www.intigriti.com/researchers/blog

When reading HackerOne or BugBoard, search for:
...
### Search Queries For Logic Bug Writeups

Use these queries on Google, BugBoard, HackerOne Hacktivity, Medium, and personal blogs:

```text
business logic bug bounty
race condition bug bounty
coupon abuse bug bounty
payment bypass bug bounty
subscription bypass bug bounty
privilege escalation organization bug bounty
invite bypass bug bounty
account takeover business logic
workflow bypass bug bounty
approval bypass bug bounty
refund logic bug bounty
cart manipulation bug bounty
multi tenant IDOR bug bounty
role downgrade bypass bug bounty
read only user privilege escalation
API mass assignment bug bounty
BOLA BFLA BOPLA bug bounty
webhook replay business logic bug bounty
SSO organization invite bypass bug bounty
```

### CTF And Practice Platforms For Logic Bugs

Most CTFs are payload-focused, so do not depend on them alone. For logic bugs, prioritize realistic web labs and intentionally vulnerable SaaS/API apps.
...
- TryHackMe web/API rooms: https://tryhackme.com/
- Root Me web challenges: https://www.root-me.org/
- WebSec.fr: https://websec.fr/
- OWASP crAPI locally.
- OWASP Juice Shop locally.
- VAmPI locally.

Best practice is to build your own small SaaS workflow too:
...
Then intentionally add weak authorization and state validation, find the bugs, and fix them. Building a vulnerable workflow changes your mindset faster than reading alone.

### How To Read A Logic Bug Writeup

Do not only copy the parameter or endpoint. Extract the mental model.

For every writeup, answer:

1. What was the business rule?
2. What state was the object supposed to be in?
3. What role was supposed to perform the action?
4. What object ownership or tenant boundary existed?
5. What assumption did the developer make?
6. What step did the hunter skip, repeat, reverse, race, or replay?
7. What exact impact did this create?
8. What made the report valid instead of just weird behavior?
9. Where else could the same pattern appear?
10. How would I test this in my next target?

After reading, rewrite the bug in this format:
...
Similar places to test:
```

### State-Machine Mindset

For each workflow, draw the normal state flow.

Example:

```text
created -> pending -> approved -> paid -> fulfilled -> refunded -> cancelled
```

Then attack the state machine:

```text
Can I skip pending?
Can I approve as the wrong user?
Can I approve after cancellation?
Can I pay once and fulfill twice?
Can I refund after transfer?
Can I cancel after receiving the benefit?
Can I use a stale token from an old state?
Can I race approval and cancellation?
Can I trigger fulfillment before payment is finalized?
Can I replay a webhook?
Can I use mobile API to perform a blocked web action?
Can a removed user still act?
Can a read-only user mutate through another endpoint?
Can Org B act on Org A's object?
```

### Best Features For Logic Bugs
...
If an app has money, roles, tenants, approvals, external integrations, or AI actions, it is worth deep business-logic testing.

### Daily Logic Bug Drill

Pick one workflow and fill this out:
...
Cross-user tests:
Cross-tenant tests:
Expected impact if broken:
```

### Logic Bug Hypothesis List

Use this when testing a real target:

- Can I create an object as low privilege and upgrade its properties later?
- Can I change hidden fields with PATCH/PUT?
- Can I assign myself to a role, team, project, or organization?
- Can I invite myself through another user's organization?
- Can I reuse an invite after being removed?
- Can I accept an invite with a different email than the invited one?
- Can I change email after verification or SSO linking?
- Can I downgrade plan after using premium features?
- Can I apply a coupon multiple times?
- Can I race credit usage?
- Can I refund and still keep the benefit?
- Can I cancel after fulfillment?
- Can I replay a payment or webhook event?
- Can I access an export generated by another user?
- Can I modify notification, report, or integration settings across tenants?
- Can I perform admin-like actions through mobile or old API?
- Can I use a stale session after role downgrade?
- Can I use an API key after user removal?
- Can AI search leak another user's documents?
- Can AI automation perform an action my role cannot perform directly?

### Mindset Upgrade

A normal tester asks:

```text
What parameter can I tamper with?
```

An elite logic tester asks:

```text
What rule is this feature supposed to enforce, and which path lets me reach a state the business never intended?
```

That is the shift. You are not only attacking requests. You are attacking the product's assumptions.
